...
ProxyClient::ProxyClient(
        const dds::xrce::CLIENT_Representation& representation,     // dds::xrce:Client 表示 是个常量
        Middleware::Kind middleware_kind,                           // 中间件种类？
        std::unordered_map<std::string, std::string>&& properties)  // 性质 hash表
    : representation_(representation)               // 列表初始化
    , objects_()                                    // objects_  初始化为空
    , session_(SessionInfo{representation.client_key(), representation.session_id(), representation.mtu()}) // 用client的表示里的client_key、session_id 和 mtu初始化session信息
    , state_{State::alive}                          // 状态初始化为alive
    , timestamp_{std::chrono::steady_clock::now()}  // 时间戳定位现在
    , properties_(std::move(properties))            // 将性质强转为右值引用
{
        ...
        const dds::xrce::ObjectVariant& object_representation)
{
    ...
    result.status(dds::xrce::STATUS_OK);    // 设置status为创建成功的status_ok
    result.implementation_status(0x00);     // 

    /* Compute ObjectId. */ // 根据prefix和representation来计算object_id
    dds::xrce::ObjectId object_id;
    object_id[0] = objectid_prefix[0];
    object_id[1] = (objectid_prefix[1] & 0xF0) | object_representation._d();

    /* Check whether object exists. */ // 检测object是否存在
    std::unique_lock<std::mutex> lock(mtx_);        // 把互斥锁上锁
    auto it = objects_.find(object_id);             // 根据object_id寻找object
    bool exists = (it != objects_.end());           // exists表示是否找到

    /* Create object according with creation mode (see Table 7 XRCE). */
    if (!exists)    // 如果不存在 则创建object
    {
