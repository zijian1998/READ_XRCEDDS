...
uint16_t uxr_prepare_output_stream(
        ...
        uxrStreamId stream_id,          // 流id
        uxrObjectId entity_id,          // 实体id
        ucdrBuffer* ub,                 // uxcr buffer
        uint32_t data_size)             // 数据大小
{
    uint16_t rv = UXR_INVALID_REQUEST_ID;       // 初始化为无效的request

    size_t payload_size = WRITE_DATA_PAYLOAD_SIZE + data_size;      //payload大小从4（write_data的payload大小）增加一个数据的大小
    ub->error = !uxr_prepare_stream_to_write_submessage(session, stream_id, payload_size, ub, SUBMESSAGE_ID_WRITE_DATA,
                    FORMAT_DATA);   // 准备写子消息的stream
    if (!ub->error) // 如果成功
    {
        WRITE_DATA_Payload_Data payload;
        rv = uxr_init_base_object_request(&session->info, entity_id, &payload.base);        // 初始化base object 的request
        (void) uxr_serialize_WRITE_DATA_Payload_Data(ub, &payload);                         // 序列化WRITE——DATA的payload的数据

        OnFullBuffer on_full_buffer = ub->on_full_buffer;
        void* args = ub->args;
        ucdr_init_buffer(ub, ub->iterator, (size_t)(ub->final - ub->iterator));             // 用偏移重新初始化buffer
        ucdr_set_on_full_buffer_callback(ub, on_full_buffer, args);                         // 设置buffer满的时候的回调
    }
